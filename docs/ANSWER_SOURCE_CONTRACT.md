...
**Prior drafts:** v1, commit `6443eb1468c1dc60dd62b36672422424e5bced1e`; v2, commit `9794d0a00e2e8baa9549cf25c0e2c0a150175438`; candidate v1, commit `983dcf37a1ae8755cc4f8730cc8cada716e6d901` — this final wording-only pass narrows the `_direct_persona_answer()` caller claim, makes the post-chain English-replacement gate source-agnostic, scopes the English-alignment invariant to the three pre-response substitution mechanisms, reconciles `learner_stated_location` consumers with §10, and broadens same-response coordination terminology beyond `frame_text` (see revision notes inline).
...
`counter_reply` and `frame_text` are produced by **distinct selection mechanisms**: the priority chain in §4 runs first, then frame selection runs afterward using the pre-turn `current_engine` (read separately from answer generation — see §17). They are not fully independent, however — answer classification computed while producing this turn's `counter_reply` can set flags that later code in the **same** handler invocation reads and acts on, for the **same** response. This same-response effect is not always a `frame_text` change specifically; depending on the flag, it can instead affect frame **selection** (a same-engine or bridge-target constraint without necessarily changing the literal text), a **`frame_text` override** (the literal text is rewritten), later **response assembly**, or a **discovery-panel/other UI-affordance field**. Verified same-turn examples (§9, §11, §17):

* `_confusion_about_app_q` (set by several §4/§9 branches) — read at line 11926, gating **discovery-panel/blue-question display logic** for this response. This does **not** by itself change `frame_text`; its documented effect is scoped to the discovery-panel/UI-affordance field it gates.
* `_noisy_location_clarify` (set by Priority 16, §3.2/§4) — read via `locals().get("_noisy_location_clarify", False)` at line 11577, driving a same-response **`frame_text` override** that rephrases the location question.
* The pending-frame-commitment guard's same-engine preference (`listening_move_reason = "offtopic_pending_frame"`, line 10706) — a same-response **frame-selection constraint**, consumed by frame selection to keep the next frame within the current engine.
* `force_travel_bridge` (computed at lines 9330–9335 from `_should_route_to_travel()`, sharing its underlying trigger with Priority 5's volunteered-travel-intent branch) — consumed at line 10604 (`elif force_travel_bridge:`) as a same-response **travel-frame selection**, setting `chosen` (this turn's `frame_text`) via `_select_next_frame_bridge()` directly inside the same response (§11).

**E4 is a distinct coordination mechanism, not the only one, and operates on a different axis:** it transports a *future* engine value (`_e4_engine_handoff`, §15) via `response["state_update"]["current_engine"]` for **frame selection on the following request**, rather than changing the current turn's `frame_text` directly. The examples above are same-turn, in-process flag/variable reads; E4 is cross-turn, via the `state_update` transport contract documented fully in `docs/CONVERSATION_ARCHITECTURE.md`/`docs/STATE_CONTRACT.md`.

```text
learner text
→ classification (last_turn_was_answer, user_asked_question, confusion/example/meaning/rr signals)
1. → priority-ordered answer-source resolution: a source candidate (zh, en) is chosen (§4)
2. → E4 eligibility is computed from that candidate, before any post-chain replacement (§15)
3. → deduplication, the exact-repeat guard, and repair escalation may each replace the Chinese;
     whichever of these replaces the Chinese also keeps its own paired English aligned (§12, §13)
     — this is the "pre-final-ASR-repair answer" (assigned to response["counter_reply"] at line
     11817; not a "fully finalised" value, since step 6 below can still change it)
4. → pinyin is derived from the Chinese as it stood after step 3, i.e. after repair escalation (§14)
5. → response fields and working-memory state (last_counter_reply, recent_persona_replies) are
     assembled from the pre-final-ASR-repair answer
6. → the final ASR-junk text repair pass may still modify response["counter_reply"]'s Chinese only (§3.3(4))
7. → this last pass does not recompute counter_reply_en, counter_reply_pinyin, or the
     already-assembled working-memory fields from step 5 — those can reference the
     pre-final-ASR-repair Chinese even after step 6 changes it
```
...
Four mechanisms, all running strictly after the priority chain has produced (or failed to produce) `_counter_result`, each capable of **replacing** the Chinese text already assigned to `_counter_reply`. The first three are **pre-response substitution mechanisms** (they run before the response is assembled/serialised and are source-agnostic — any answer source's Chinese can reach them, §13); the fourth is a distinct, **later, response-level** mechanism:

1. **Deduplication substitution** — `_dedupe_persona_answer()`, called from the handler at lines 10352–10367 (§8/§12). *Pre-response substitution mechanism.*
2. **Exact-repeat belt-and-suspenders guard** — lines 10372–10374, immediately after (1). *Pre-response substitution mechanism.*
3. **Repair escalation override** — lines 10402–10426, gated on `_repair_attempt_count >= 2`, independent of `_prev_counter_reply`/mirror-topic state and independent of `user_asked_question`. *Pre-response substitution mechanism.*
4. **Final ASR-junk text repair** — lines 12407–12415, `response["counter_reply"] = _repair_asr_junk_text(_cr_final)` (string case) or `_cr_final["zh"] = _repair_asr_junk_text(...)` (dict case). *Later, response-level mechanism — not one of the three pre-response substitution mechanisms above.* This is the **second and last** direct assignment site to `response["counter_reply"]` found anywhere in the file (the first is the initial assembly at line 11817). It runs after frame selection, discovery-panel assembly, and session-end handling — immediately before the response is serialised and sent (line 12418). It strips known ASR-junk fragments (e.g. `"等你等…"`) from whatever Chinese text is already in `response["counter_reply"]`, whatever path produced it.
...
* Line 11817 — `response["counter_reply"] = _counter_reply` — the primary transport of **the pre-final-ASR-repair answer** (post-priority-chain, post-dedup, post-repair-escalation, but not yet post-final-ASR-junk-repair, §3.3(4)) into the response payload.
...
The chain is **not** a single flat `if/elif`. It is five separate structural groups, in this exact top-to-bottom execution order. Precision on empty-result semantics (verified directly against the Python source, not inferred): in Groups 1 and 2, each is a single `if`/`elif` chain keyed on the **trigger condition itself**, not on whether the callee inside the matched branch returns a usable result. Consequently, once a condition evaluates `True` and its `elif` branch is entered, **no lower `elif` in the same group is ever evaluated**, even if the code inside the matched branch itself decides not to assign `_counter_result` (e.g. because the branch's own helper returned an empty/`None` result).

This **blocks only the lower branches within that same group** — it is not a dead end for the turn as a whole. Precisely:

* A matched Group 1 condition (Priorities 1–5) whose callee returns empty blocks only the remaining Group 1 `elif` branches. Group 2 (Priorities 6–17) still executes afterward, exactly as if Group 1 had never matched anything, because Group 2 is gated on `_counter_result is not None` (line 10027) independently re-checking the (still-`None`) value.
* A matched Group 2 condition whose callee returns empty blocks only the remaining Group 2 `elif` branches in the same way. Groups 3, 4, and 5 (Priorities 18, 19, 20–22b) still execute afterward, since each is its own independent `if _counter_result is None:` gate (§4).
* Only if **every** group — 1 through 5 — fails to populate `_counter_result` does the turn end this priority chain with `_counter_result` still `None` (`_counter_reply` becomes `None`, subject only to the post-chain repair-escalation override, §4.3, which does not require a pre-existing `_counter_result`).

An earlier draft of this document described empty-but-matched branches as "dead-ending the turn." That wording is corrected throughout this document to **"blocks lower branches within the same group"**, since the remaining four (or fewer) groups are unaffected and still run.
...
|---|---|---|---|---|---|---|
| 1 | Frustration/insult repair | `_is_frustration_or_insult(answer_text)` (line 9903) | `_frustration_repair_reply()` | `content/recovery_phrases.json` (`use=frustration_repair`) | Same tuple | If the callee returns empty, this **blocks lower Group 1 branches (Priorities 2–5) only** — the trigger condition, not the return value, drives the `elif` chain. Groups 2–5 still run afterward |
| 2 | Learner disclosure empathy | `_is_learner_disclosure(answer_text)` (line 9909) | `_disclosure_empathy_reply()` | `content/recovery_phrases.json` (`use=learner_disclosure_empathy`) | Same tuple | Same group-local blocking as Priority 1 — blocks only Priorities 3–5 |
| 3 | Persona challenge reply | `_is_persona_challenge(answer_text)` (line 9917) | `_persona_challenge_reply()` | `content/recovery_phrases.json` (`use=persona_challenge`) | Same tuple | Same group-local blocking — blocks only Priorities 4–5 |
| 4 | Responsive food answer | `_responsive_food_answer` flag (line 9925; computed lines 9227–9234) | `_food_responsive_reply()` | Extracted food items / inline pool | Same tuple | Same group-local blocking — blocks only Priority 5 |
| 5 | Volunteered travel intent | `not user_asked_question and _has_volunteered_travel_intent(answer_text)` (line 9936) | `_travel_intent_followup()` | `content/recovery_phrases.json` (`use=travel_intent_followup`) | Same tuple | Last Group 1 branch; an empty result here simply ends Group 1 with `_counter_result` still `None` — Group 2 runs next regardless |
...
| 8 | Meaning recovery | `_is_meaning` (line 10033) | `_meaning_recovery_reply()` | `_MEANING_RECOVERY_TABLE` (inline) | Same tuple | If `last_partner_frame_text` is empty, this blocks lower Group 2 branches (Priorities 9–17) only — Groups 3–5 still run afterward |
| 9 | Example request → clarify | `_is_example` (line 10038) | `_clarify_app_question()` | `last_partner_frame_text` restated | Same tuple | Same group-local blocking — blocks only Priorities 10–17 |
| 10 | Repeat/slower request → clarify | `_is_rr` (line 10046) | `_clarify_app_question()` | `last_partner_frame_text` restated | Same tuple | Same group-local blocking — blocks only Priorities 11–17 |
...
| 15 | App-question clarification (no prior counter_reply) | `not _prev_counter_reply` + confusion signal + `not user_asked_question` + `not _confirmed_re_ask` (lines 10126–10131) | `_clarify_app_question()` | `last_partner_frame_text` restated | Same tuple | If frame text empty, this blocks lower Group 2 branches (Priority 16–17) only — Groups 3–5 still run afterward |
| 16 | Noisy location (control-only, §3.2) | `"CITY" in slot_names` + no resolvable location + other guards (lines 10141–10148) | *(none — flag-only)* | n/a | n/a | This branch itself never assigns `_counter_result` (by design — its intended effect is a later `frame_text` override), and it blocks lower Group 2 branches (Priority 17) exactly as any other matched Group 2 branch would. It does **not** guarantee the turn ends without a `counter_reply` — Groups 3–5 still execute afterward with `_counter_result` still `None`, and Group 5 (Priorities 20–22b, the mirror bank/prefix path) commonly populates a `counter_reply` on the same turn that this flag is set |
...
| 21 | General answer prefix (`_answer_user_question_prefix`) | Mirror miss or `not user_asked_question` (line 10242); internal sub-chain (§5–§7, §16) | `_answer_user_question_prefix()` | Varies by sub-step | Varies | **Not unconditionally non-`None`.** Three explicit early-`return None` guards exist ahead of the fallback tail (empty text, line 4965; neither a generic question nor a direct-persona question, line 4970; a confusion signal not matching a recognised persona-question prefix, line 4982). Only if execution passes all three does it reach the fallback tail, where `_persona_limitation_reply()` (line 5089) is an unconditional final fallback that always returns a non-empty result. See §16 for the fallback tail and §4.2 Priority 22b for why the confusion-signal early return matters |
...
| 22b | Confusion signal with question mark (post-prefix fallback) | `user_asked_question and not _counter_result and _is_confusion_signal(answer_text)` (lines 10273–10276) | `_clarify_app_question()` | `last_partner_frame_text` restated | Same tuple | Sequential `if`, explicitly guarded on `not _counter_result` so it never overwrites 22a's result. This branch is reachable precisely because Priority 21 is not unconditionally non-`None` (§4.2 Priority 21): when `_is_confusion_signal(answer_text)` is true and the text does not start with a recognised persona-question prefix, `_answer_user_question_prefix()` itself returns `None` at line 4982, before ever reaching its `_persona_limitation_reply()` fallback tail — leaving `_counter_result` as `None` and `user_asked_question` still `True`, exactly the condition 22b checks for. If 22b's own guard also fails, `_counter_result` remains `None` for the rest of the priority chain |
...
`_direct_persona_answer(t, persona, recent_replies=None)` (defined starting at `scripts/ui_server.py:3057`) returns `Optional[str]` (**Chinese only**). Every **production answer-producing caller** wraps the returned Chinese with a `我呢，` prefix (unless it already starts with `我`) as required and resolves English through `_persona_answer_en()` — this holds for all four such call sites (Priorities 7, 12, and the internal direct-persona sub-step of Priority 21, §16). **`_is_direct_persona_question(t)` is a separate, non-answer-producing caller** (§16): it is a pattern-probe that calls `_direct_persona_answer(t, None)` with `persona=None` solely to test whether *any* intent family would structurally match the text; it does not publish the returned Chinese, does not add the `我呢，` prefix, and does not call `_persona_answer_en()`. This function is a **single, strictly first-match sequence** of 42 recognised intent families.
...
**Practical consequence, narrowed to what the evidence actually supports:** for the specific subset of questions `_detect_reverse_fact_intent()` can classify (marriage, age, hometown_food, hometown_special, work_duration, work_reason, job, hometown_where), the Chinese answer is produced by whichever of the live mechanisms actually matches first — `_direct_persona_answer` (Priorities 7/12, §5), the mirror bank (Priority 20, §7.1), or `_answer_user_question_prefix`'s internal sub-chain (Priority 21, §5/§16) — never by `_reverse_fact_answer()` itself. `_reverse_fact_answer_en(intent, ...)` is the only reverse-fact *function* actually reachable in production, called from `_persona_answer_en()` using the intent computed from the **question text**, independent of which live mechanism produced the Chinese candidate. This statement is scoped to reverse-fact-classifiable questions only — it is not a claim that all production Chinese answers, in general, come from only these two sources. The wider answer system also produces Chinese via recovery/repair functions (§9), E3 working memory (§8), phrase-bank lookups (§13 stage 1), inline city/food/feature pools (§5, §10), and the three unsupported-question fallbacks (§16), none of which are reverse-fact-classifiable inputs to this specific mechanism.

### 7.3 Answering from persona facts vs. recent conversational context

`_direct_persona_answer` and the reverse-fact functions answer **regardless of conversation history** — neither reads `recent_persona_replies` or any other rolling conversational-context state. This is the axis that actually distinguishes them from E3 working memory (§8), which reads `recent_persona_replies` instead of persona-adjacent sources.

This is a narrower claim than "answer strictly from persona JSON." `_direct_persona_answer` is **not** strictly persona-JSON-driven: per §5's intent-family table and §5's "inline Chinese fallback content" note, a majority of its 42 families read `profile`/`discoverable_facts`/`voice_lines`/`distance_profile` (persona JSON) only as a **preferred** source, falling back to hardcoded inline Chinese templates, the three module-level city pools (`_CITY_LIKE_POOL`, `_CITY_FOOD_POOL`, `_CITY_FEATURE_POOL`), or a `_persona_deflect("<topic>", t)` phrase-bank call (marriage, children, age) when persona JSON has no matching field. `_reverse_fact_answer`/`_reverse_fact_answer_en` likewise read only `profile`/`discoverable_facts`/`voice_lines` (no inline pools of their own) — but `_reverse_fact_answer()` itself is not production-reachable at all (§7.2), so this narrower persona-JSON-only characterisation applies only to the English-side `_reverse_fact_answer_en()`, not to a Chinese-producing path.
...
| Learner-stated open-world location | `learner_stated_location` in per-session `conversation_state`/`state_update` | Written at lines 12127–12135: extracted via `_extract_open_world_location(answer_text, frame_is_residence=True)` when `last_answer_fid in _RESIDENCE_QUESTION_FRAME_IDS` (`f_from_where`, `f_live_where`, `frame.location.live_question`); otherwise carried forward from its own prior value | Read back into `_prev_learner_loc` for its own carry-forward (line 12127); feeds `_new_lps`/`last_place_subject` (line 12141–12142, a third, distinct state field); read into a diagnostics-only capture at line 9269. **Not read by any answer-source function** — not `_place_from_question_context`, not `_direct_persona_answer`, not any function in §4–§16 |

**`learner_memory["lives_in"]` is a separate, persistent, cross-session field, not this same field under another name.** It is written by an entirely independent extraction pipeline in `scripts/learner_memory_capture.py::capture_from_turn()`, which scans `answer_text` against `_KNOWN_PLACES_CANONICAL`/`_PLACE_ALIASES` (a *known*-place-only match, unlike `learner_stated_location`'s open-world extraction) when the frame is one of `_LIVE_FRAMES` (`frame.location.live_question`, `f_live_where` — a narrower set than `_RESIDENCE_QUESTION_FRAME_IDS` above, since it omits `f_from_where`), and persisted via `_lm_save()` outside the per-request `conversation_state`. It is read by several answer-source-adjacent functions in `scripts/ui_server.py` (lines 1626, 1646, 1660, 5141, 9622–9627, 11675) as `mem.get("lives_in")`, typically with a `hometown` fallback.

**No code path in this repository copies, persists, or derives one of these two fields from the other.** They are computed by two independently-implemented extraction functions, triggered by overlapping-but-not-identical frame-ID sets, from the same `answer_text` in the same turn, and stored in two different places (session-scoped `conversation_state` vs. the persistent `learner_memory` store). It is possible for them to diverge in the same turn — e.g. an unknown/open-world place (not in `_KNOWN_PLACES_CANONICAL`) can populate `learner_stated_location` while `learner_memory["lives_in"]` remains unset for that same answer, because `learner_memory_capture.py`'s extractor requires a known-place match. A place the learner mentions can be **stored** as `learner_stated_location` regardless of encyclopedic coverage; the system cannot **describe** that place beyond the generic fallback templates in §10 above unless it is a persona's own city/hometown or a key in one of the three hardcoded pools — this description-capability statement is independent of whether `learner_memory["lives_in"]` was also set.
...
|---|---|---|---|---|---|---|
| Volunteered travel intent | `not user_asked_question and _has_volunteered_travel_intent(answer_text)` | Implicit in the templated reply | Yes — template includes a follow-up question | No | Separate: `force_travel_bridge` (lines 9330–9335), computed from `_should_route_to_travel()`, which itself calls `_has_volunteered_travel_intent(answer_text)` — the same underlying trigger as this `counter_reply` branch — bridges to a travel-engine frame **in the same `/api/run_turn` response**, not a following request | The follow-up question is inside `counter_reply`; when `force_travel_bridge` is also true, `frame_text` **on the same turn** is set to the travel-bridge frame via `_select_next_frame_bridge()` at line 10604–10618 (verified by direct line citation, not inferred) — this is a same-turn effect, corrected from an earlier draft's claim that `frame_text` on the same turn is unaffected |
...
**Can a source-provided English value be replaced later? The post-chain replacement gate is source-agnostic.** Whether an answer's Chinese was originally produced by the mirror bank, E3, a recovery/repair function, a phrase-bank lookup, `_direct_persona_answer` (via `_persona_answer_en()`), or an unsupported-question fallback, it can reach deduplication (§12) as long as its Chinese satisfies the outer exact-match gate (`_counter_reply.strip() in _dedup_pool`) — the gate tests only the Chinese string, never which function produced it. Once a candidate reaches deduplication:

* **Deduplication and the exact-repeat guard** (§12) replace the Chinese and resolve the *replacement's* English via `_persona_answer_en()`, regardless of how the *original* English was produced — including when the original was mirror-bank or E3 source-provided English that never itself passed through `_persona_answer_en()`.
* **Repair escalation** (§9) replaces both `_counter_reply` and `_counter_reply_en` together as a new literal `(zh, en)` pair, not by calling `_persona_answer_en()` again.
* **The final ASR-junk repair pass** (§3.3(4)) changes only the response-level Chinese text and does not update `counter_reply_en` or `counter_reply_pinyin` at all — this is outside the three mechanisms above (§18).

This is distinct from, and should not be confused with, the *original* candidate's English-resolution mechanism: §13's five-stage `_persona_answer_en()` precedence table describes only how the **initial** English for `_direct_persona_answer`-sourced Chinese is resolved before any replacement; it does not describe, and is not the only path for, the English attached to a **replacement** Chinese string produced by dedup or the exact-repeat guard.
...
**Enforced invariant (narrowed, replacing any broader "always synchronised" claim):** *The three pre-response substitution mechanisms — deduplication, the exact-repeat guard, and repair escalation — replace or recompute the paired English whenever they replace Chinese.* Demonstrated for dedup and the exact-repeat guard (test-covered by `tests/test_zh_en_synchronisation.py::TestDeduplicationEnglishSync`) and for repair escalation (self-consistent literal pairs, not independently test-asserted for the "recompute" property since no recomputation call occurs — the pair is simply assigned together).

**The final ASR-junk repair is a separate, later, response-level mechanism, outside this invariant.** It runs after all three pre-response substitution mechanisms above (§3.3(4), §12), changes only the Chinese text of `response["counter_reply"]`, and does not recompute `counter_reply_en`, `counter_reply_pinyin`, or the already-assembled `last_counter_reply`/`recent_persona_replies` working-memory fields. This remains a documented gap, not a covered case of the enforced invariant.
...
| `learner_stated_location` (session-scoped) | **Not directly read by `_place_from_question_context()`, and not directly read by any answer-producing function inventoried in §§4–16.** Its exact in-file consumers, all outside the answer-source functions themselves, are: (1) its own session-state carry-forward (`_prev_learner_loc` read at line 12127, re-written into `_su["learner_stated_location"]` at line 12135); (2) seeding the **separate** `last_place_subject` state field — this turn's freshly-extracted value (`_learner_loc_this_turn`) feeds `_new_lps` at line 12141–12142, which is then written to `_su["last_place_subject"]` at line 12153; (3) a diagnostics-only capture at line 9269 | Written at lines 12127–12135 from `_extract_open_world_location()`, gated on `_RESIDENCE_QUESTION_FRAME_IDS` | Distinct from `learner_memory["lives_in"]` below — see §10 for the full separation; no code path derives either field from the other. `last_place_subject` is a separate state field, not another name for `learner_stated_location` — it can also be populated from the fixed known-place scan (`_LPS_PLACES`, lines 12105–12119) independently of any learner-stated open-world location |
| `learner_memory["lives_in"]` (persistent, cross-session) | Read by several place-answer-adjacent functions as `mem.get("lives_in")` (lines 1626, 1646, 1660, 5141, 9622–9627, 11675), typically with a `hometown` fallback — these are answer-source-adjacent construction helpers, not the `_place_from_question_context()` function itself | Written by `scripts/learner_memory_capture.py::capture_from_turn()`, an independent extraction pipeline gated on `_LIVE_FRAMES`, persisted via `_lm_save()` | Separate storage (persistent `learner_memory` module vs. session `conversation_state`), separate extraction logic, separate frame-ID gating from `learner_stated_location` above |
...
* **The three pre-response substitution mechanisms — deduplication, the exact-repeat guard, and repair escalation — replace or recompute the paired English whenever they replace Chinese** — demonstrated for dedup/exact-repeat (test-covered, §13) and self-consistent by construction for repair escalation. This invariant is scoped to those three mechanisms only; the later, response-level final ASR-junk repair pass (§3.3(4)) is explicitly outside it and remains a documented gap.
...
* **Source-order changes altering unrelated behaviour** — because Groups 1 and 2 are single flat `elif` chains where a matching-but-empty branch blocks only the lower branches within that same group (§4), reordering or adding a new condition anywhere in those groups risks silently suppressing an existing lower-priority answer source *within that group* (Groups 3–5 are unaffected). *Observed* as a structural property, not a specific historical incident.
...
* **Answer sources returning different tuple shapes or assumptions** — `_direct_persona_answer` returns `zh`-only, requiring every answer-producing call site to independently wrap it and call `_persona_answer_en()` (four distinct answer-producing call sites, not centralised once — excluding the non-publishing `_is_direct_persona_question` pattern-probe caller, §5), whereas most other sources return `(zh, en)` directly. *Observed.*
...
* **Learner question receives no answer:** confirm `last_turn_was_answer` was true and check whether the question text matched a Group 1 or Group 2 trigger with an *empty* callee result — per §4's corrected semantics, this blocks only the remaining branches in that same group; Groups 3–5 still run afterward, so also check whether any of Priorities 18–22b subsequently produced (or failed to produce) an answer before concluding the turn genuinely ended with no `counter_reply`.
...
| Responsive food / travel intent | `_food_responsive_reply`, `_travel_intent_followup` | Extracted text / `content/recovery_phrases.json` | Tuple's own `en` | Volunteered travel intent may set `force_travel_bridge` and select the same response's travel frame; responsive-food handling does not set this signal | `tests/test_open_world_food_and_location_fixes.py`, `tests/test_regression_place_travel_reverse.py` |
