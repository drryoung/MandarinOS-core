...
**Prior drafts:** v1, commit `6443eb1468c1dc60dd62b36672422424e5bced1e`; v2, commit `9794d0a00e2e8baa9549cf25c0e2c0a150175438` — this revision corrects group-local (not whole-chain) blocking semantics, Priority 21/22b return-path consistency, same-turn answer/frame coordination beyond E4, volunteered-travel same-turn frame timing, final-ASR-repair finalisation wording, reverse-fact overgeneralisation, and the `learner_stated_location`/`learner_memory["lives_in"]` field distinction (see revision notes inline).
...
`counter_reply` and `frame_text` are produced by **distinct selection mechanisms**: the priority chain in §4 runs first, then frame selection runs afterward using the pre-turn `current_engine` (read separately from answer generation — see §17). They are not fully independent, however — answer classification computed while producing this turn's `counter_reply` can set flags that frame selection, running later in the **same** handler invocation, reads and acts on for the **same** turn's `frame_text`. Verified same-turn examples (§9, §11, §17):

* `_confusion_about_app_q` (set by several §4/§9 branches) — read at line 11926, gating discovery-panel/blue-question display logic for this response.
...
* The pending-frame-commitment guard's same-engine preference (`listening_move_reason = "offtopic_pending_frame"`, line 10706) — consumed by frame selection in the same response to keep the next frame within the current engine.
* `force_travel_bridge` (computed at lines 9330–9335 from `_should_route_to_travel()`, sharing its underlying trigger with Priority 5's volunteered-travel-intent branch) — consumed at line 10604 (`elif force_travel_bridge:`) to select `chosen` (this turn's `frame_text`) via `_select_next_frame_bridge()`, directly inside the same response (§11).
...
| `learner_stated_location` (session-scoped) | **Not read at all by any answer-source function.** `_place_from_question_context()` reads only its own `text`/`recent_replies` parameters (§10); this field's only in-file consumers are its own carry-forward (line 12127) and a diagnostics-only capture (line 9269) | Written at lines 12127–12135 from `_extract_open_world_location()`, gated on `_RESIDENCE_QUESTION_FRAME_IDS` | Distinct from `learner_memory["lives_in"]` below — see §10 for the full separation; no code path derives either field from the other |
...
| Responsive food / travel intent | `_food_responsive_reply`, `_travel_intent_followup` | Extracted text / `content/recovery_phrases.json` | Tuple's own `en` | May set `force_travel_bridge`, which sets this **same turn's** `frame_text` (§11), not a following turn's state | `tests/test_open_world_food_and_location_fixes.py`, `tests/test_regression_place_travel_reverse.py` |
