...
Eight class-D superseded documents relocated to `docs/archive/superseded/` in approved Phase C2D2 (see §17.13); `docs/specs/CONVERSATION_ARCHITECTURE_INDEX.md` retains a live navigation index and its eight relative links to those files were repointed to the new archive path. Thirty-two class-C historical specifications relocated to `docs/archive/specs/` in approved Phase C2D3-core (see §17.14); `docs/specs/CONVERSATION_ARCHITECTURE_INDEX.md` had 18 further relative links repointed to the new archive path, and remains at its current path pending the separately reviewed Phase C2D3-review batch (with five other class-C specifications carrying a code-comment or AI-bootstrap reference). Three current class-B specifications and the remaining class-E/F documents are unaffected.
