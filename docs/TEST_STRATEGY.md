...
| `python -m unittest -v` (repository root, bare — the literal `unit-tests` job command in `.github/workflows/coverage_scan.yml`) | See §5 and the CI-scope note below this table — do **not** assume this executes the pytest-style core/`live_server` tiers described above; `collect_ignore` has no effect on this command | Everything not selected by `unittest`'s own discovery — this is materially different from, and generally much smaller than, "everything pytest would collect" | None declared, but see the CI-scope note below for a discovery-root risk that must be verified against real CI logs | Narrow — only `unittest.TestCase`-derived tests actually run; see §5 |
| `python -m unittest -v tests/test_coverage_affordance.py tests/test_open_card_trace_integration.py` (the `coverage_scan` job's second, file-path-qualified command) | Exactly the `unittest.TestCase` classes in those two named files — 4 tests, verified locally as exit 0/`OK` | Every other file in the repository (not passed as an argument) | None | A (real Python, `unittest.TestCase`-based) |
...
**CI-scope note — read before relying on "CI is green" as evidence of anything beyond exactly what was executed.** `collect_ignore` in `tests/conftest.py` controls **pytest collection only**; `python -m unittest` never reads `conftest.py` at all, so it neither honours `collect_ignore` nor any pytest marker, fixture, or `pytest_configure`/`pytest_runtest_setup` hook — those mechanisms simply do not exist from `unittest`'s point of view. `unittest`'s test loader only recognises `unittest.TestCase` subclasses; a `test_*.py` file consisting entirely of plain pytest-style `def test_foo(fixture):` functions or fixture-parametrised plain classes (e.g. `tests/test_progress_store.py`, `tests/test_zh_en_synchronisation.py` — see §9) is still `__import__`-ed by `unittest`'s discovery (a file being importable does not mean its pytest-style functions ran), contributes **zero** test cases to the run, and produces no error and no visible indication that it was skipped — the "Ran N tests" total simply does not include it. Green CI proves only the exact `unittest.TestCase` cases actually enumerated and executed, nothing about files that were merely importable, and nothing about the pytest-style `-m "not live_server"`/`-m "live_server"` tiers documented elsewhere in this section — those tiers are not invoked by `.github/workflows/coverage_scan.yml` at all.

A locally-reproduced, environment-disclosed finding that materially affects how the bare `unit-tests` job command should be trusted: this command was run in a local Windows/Python 3.14 environment (CI's `coverage_scan.yml` pins `ubuntu-latest`/Python `3.10` — a disclosed environment difference not independently re-verified against that exact combination). In that local environment, the literal bare `python -m unittest -v`, executed from the repository root with no arguments (exactly matching the CI step), discovered and ran **zero tests** — because `tests/` has no `__init__.py` and `unittest`'s default discovery, started from `.`, did not recurse into it. This was confirmed twice at the repository, and independently reproduced with a minimal, repository-external two-file case (a bare `test_*.py` in a subdirectory without `__init__.py`, invisible to `python -m unittest -v` run from the parent directory, and visible only once discovery was pointed explicitly at that subdirectory or the file was moved to the current directory). Pointing discovery explicitly at the subdirectory (`python -m unittest discover -s tests -p "test_*.py" -v`) found **88** `unittest`-accounted test cases across exactly **6** modules containing `unittest.TestCase` subclasses (14 classes total): `tests/test_conversation_fixes.py`, `tests/test_coverage_affordance.py`, `tests/test_open_card_resolver.py`, `tests/test_open_card_trace_integration.py`, `tests/test_open_card_wiring.py`, `tests/test_report_asr_traces.py` — 80 of those 88 ran as real assertions ("ok"); the remaining 8 were `unittest.loader._FailedTest` import-failure entries reported as `ERROR` (overlapping several `collect_ignore` files plus two non-ignored files that `import ui_server` at module scope), and in this local run all 8 failed with the identical `ValueError: I/O operation on closed file` pattern traced to a module-level `print()` colliding with this specific sandbox's own stdout redirection — this specific error signature is disclosed as a **local sandbox artifact, not a certified CI behaviour**, since it was not verified against an actual GitHub Actions log.

**What this means, stated without overclaiming:** because the CI `unit-tests` job's literal command (bare, no `-s`, run from the checked-out repository root) is exactly what discovered zero tests locally, there is a real, unresolved possibility that the `unit-tests` job in `.github/workflows/coverage_scan.yml` also discovers and runs zero tests on `ubuntu-latest`/Python `3.10`, passing vacuously rather than exercising the 6 `TestCase`-based modules above. This was **not** independently confirmed against real GitHub Actions run logs in this pass (no access to CI logs from this environment) and must not be treated as certain. The `coverage_scan` job's second command, which names its two files explicitly, does not depend on this discovery-root behaviour at all and was confirmed locally to run its 4 tests correctly (exit 0, `OK`). Do not describe the bare `python -m unittest -v` CI step as covering the pytest-style core/`live_server` tiers, the `collect_ignore`-excluded files, or even the full set of `unittest.TestCase` modules under `tests/`, until this discovery-root question is resolved by inspecting an actual CI run's log output.
...
# CI unit-tests job — .github/workflows/coverage_scan.yml (discovery-root risk — see CI-scope note above)
python -m unittest -v

# CI coverage_scan job's second command — .github/workflows/coverage_scan.yml (file-path-qualified, verified locally: 4 tests, OK)
...
- CI-executed `unittest.TestCase` scope (see §4's CI-scope note for the full disclosure and environment caveat): pointing `unittest` discovery explicitly at `tests/` locally enumerated **88** accounted test cases across **6** modules / **14** `TestCase` classes (`tests/test_conversation_fixes.py`, `tests/test_coverage_affordance.py`, `tests/test_open_card_resolver.py`, `tests/test_open_card_trace_integration.py`, `tests/test_open_card_wiring.py`, `tests/test_report_asr_traces.py`); **80** ran as real assertions and **8** were import-failure `_FailedTest` entries (a local-sandbox-specific error signature, not certified against CI). The remaining ~66 of the 72 `tests/test_*.py` files contain **no** `unittest.TestCase` subclass at all — they are pytest-style (plain functions and/or fixture-parametrised plain classes) and, if `unittest` attempts to import them, contribute **zero** test cases silently, with no error and no visible skip notice. Whether the CI `unit-tests` job's bare, no-argument invocation from the repository root reaches this same 88-test enumeration, or instead discovers zero tests (as the literal command did in the disclosed local environment), is an open, unresolved question requiring inspection of an actual CI run's log — see §4.
...
5. Determine how the regenerated files will be made available to the deployed environment, given that Railway does not run the builder automatically — this may mean committing the regenerated files if the deployment process expects them present, or another explicit provisioning step; this document does not prescribe which, only that step 5 must be answered before deploying.
...
| Recovery phrase-bank change (`content/recovery_phrases.json`) | See the expanded requirements immediately below this table | Core suite; regenerate artifacts (`python tools/build_runtime_artifacts.py`) before any test that reads the runtime file | Real-browser verification required before shipping — see below | `docs/ASR_PIPELINE.md` §7–§8 |
...
3. Run `node tests/verify_spoken_recovery_exact_match.js`, and update its mirrored matching logic if the new phrase changes matching behaviour — but label this explicitly as **hybrid/mirrored evidence**, not behavioural proof that the real shipped client would intercept the phrase (§3.C).
...
2. CI's `unit-tests` job (`python -m unittest -v` in `.github/workflows/coverage_scan.yml`) exercises, at most, the small subset of `tests/test_*.py` files containing `unittest.TestCase` subclasses that its bare discovery actually finds — per §4's CI-scope note, this was locally observed to be **zero** files when the exact command is run unmodified from the repository root, versus 6 files/88 cases when discovery is pointed explicitly at `tests/`; which of these the real CI job experiences is unresolved without inspecting its logs. In neither case does it run the curated pytest tiers described in `tests/conftest.py` (`-m "not live_server"`, `-m "live_server"`) — those are **not** invoked by CI at all. A maintainer relying on "CI is green" as evidence for the pytest tiers, or for any pytest-style file CI merely happened to import, would be mistaken.
