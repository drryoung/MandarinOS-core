...
* **The repository does not define an installable-PWA configuration or any installed-mode-specific ASR behaviour.** There is no `<link rel="manifest">`, no Web App Manifest, no service worker, no service-worker registration call, and no code branching on `display-mode: standalone`/`navigator.standalone` anywhere in this codebase. Microphone behaviour as implemented here is governed solely by the standard browser secure-context rules and the JS check above, and this document makes no claim about how any given browser or platform would behave if a manifest enabling installation were hypothetically added — only that no such repository-defined variation currently exists to document.
...
**Fields explicitly NOT found in the payload:** `confidence`/`asr_confidence`/`recognition_confidence`, `is_spoken`, `input_mode` (as a client-sent field), `raw_transcript`, any challenge-mode marker. The server-computed `_sel_trace.input_mode` field (§16) is a same-turn, response-only heuristic, **not** sent by the client, with exactly four possible values based on which of `last_answer.submitted_text`/`.selected_option_hanzi` are populated: both populated → `"asr"`; `submitted_text` only → `"typed"`; `selected_option_hanzi` only → `"option_tap"`; neither → `"none"`. Ordinary Mechanism 1 (Chinese microphone) and Mechanism 2 (translate-assisted) submissions each populate only `submitted_text`, so **both are labelled `"typed"` by this heuristic, not `"asr"`** — the field's `"asr"` value is a historical misnomer that actually denotes an option-tap-plus-free-text combination, not microphone input, and this heuristic is not a reliable spoken-versus-typed indicator (confirmed identically in §16).
...
| EN/pinyin/frame-English changed? | **Yes, but by same-source placeholder substitution, not independent recomputation** — the single repaired `_city` value is substituted directly into whichever of the `{CITY}`/`{PLACE}` tokens are present in `frame_text`, the `{CITY}` token in `frame_pinyin`, and the `[CITY]` token in `frame_text_en` (`11686-11689`), keeping all three fields updated from the same repaired source string. This is **not** an independent English translation of `_city`, nor an independent pinyin romanisation of it — `frame_pinyin`'s `{CITY}` placeholder is replaced with the same Chinese proper-name string as `frame_text`'s, so `frame_pinyin` may end up containing a Chinese-character place name rather than its romanised form. This is the exception to the general "EN/pinyin left stale" pattern seen at the final-repair call sites below, but it is a substitution behaviour, not a recomputation behaviour |
...
Structurally identical to call site 3 in every respect (same enclosing block, same "read persisted memory → repair → substitute the repaired value into the placeholder-bearing `frame_text`/`frame_text_en`/`frame_pinyin` fields in the same block → after frame selection"), differing only in that it targets the `{HOMETOWN}`/`[HOMETOWN]` tokens specifically using `_slot_mem.get("hometown")`. As with site 3, this is same-source placeholder substitution across three fields, not an independent English translation or pinyin romanisation of the repaired hometown value.
...
Call sites 5-7 are the same mechanism documented as the final ASR-junk output-repair pass in `docs/ANSWER_SOURCE_CONTRACT.md` §3.3(4); that document's non-recomputation-of-English/pinyin finding is confirmed directly from this call site's code and is not re-derived independently here. Call sites 1-2 are genuinely input/routing-side (never touch the visible transcript directly, never persist to memory) but have **caller-dependent** before/after-selection timing, not a single fixed phase. Call sites 3-4 are the sole call sites where `frame_text`, `frame_text_en`, and `frame_pinyin` are all updated **from the same repaired source value in the same block** — this is same-source placeholder substitution across three fields, not an independent English translation or pinyin romanisation of that value, and is documented as such rather than as "recomputation."
...
**Challenge Mode changes visibility only, and the storage surface differs by element — this is not a single "stored" claim.** Hidden partner content (rows 1-4) remains available to the normal DOM/client rendering state: it is still fully populated into the DOM and into `window._sentenceHint`/`window._currentFrameText`/`window._lastPartnerSpokenText` during ordinary turn processing — this is in-memory client rendering state, not persistent storage. The ASR interim preview (row 5) is never suppressed by Challenge Mode at all. Learner and partner transcript entries remain in the client's in-memory `conversationTranscript` array regardless of the transcript panel's own hidden `display` state (row 6) — only the DOM container's visibility is hidden, not the underlying array. Learner answers continue to be submitted normally via whichever mechanism produced them (§2), unaffected by Challenge Mode. **None of this amounts to persistent storage** — persistent storage of any of this content is conditional entirely on the separate, opt-in end-session capture mechanism described in §16 (`MANDARINOS_SESSION_CAPTURE=1`), and is otherwise confined to the current page's in-memory client state.
...
**Client-intercepted spoken recovery, precisely stated:** by construction (§7, §8), it never produces an `/api/run_turn` request, and it is therefore absent from per-turn server routing, per-turn answer generation, and per-turn diagnostics (`_diag_cap`/`selector_trace`) — none of which ever run for an intercepted utterance. It **is** added to the client's own `conversationTranscript` array (via `addTranscriptEntry`, `ui/app.js:7730`), and `/api/end_session` sends that same client transcript array verbatim (§16's field-shape table above). Consequently, **when `MANDARINOS_SESSION_CAPTURE=1` is set, the intercepted-recovery entry may be persisted into the end-session session JSON** as part of that transcript array — this is not a categorical absence "under any configuration." What it remains, in every configuration, is **a client-authored transcript record of an interaction the server never observed as a semantic conversation turn** — it carries no `frame_id` progression, no server-side answer-source resolution, and no routing/classification history, unlike a genuine submitted turn.
...
This response field is **not persisted to disk** — it is returned per-request only, and would need to be captured by a client-side or diagnostics-side consumer to survive past that single response. Restating §9's exact heuristic for `_sel_trace.input_mode` here for consistency: both `submitted_text` and `selected_option_hanzi` populated → `"asr"`; `submitted_text` only → `"typed"`; `selected_option_hanzi` only → `"option_tap"`; neither → `"none"`. Ordinary Mechanism 1 (Chinese microphone) and Mechanism 2 (translate-assisted) submissions each populate only `submitted_text`, so **both are labelled `"typed"`, not `"asr"`**, by this heuristic — `"asr"` actually denotes an option-tap-plus-free-text combination, a historical misnomer that does **not** mean "this text came from the microphone." This is a further confirmation that the server has no reliable spoken-vs-typed signal (§1, §9).
...
* **Output junk repair should keep Chinese/English/pinyin synchronised.** Confirmed at the call-site level (§11): sites 3-4 substitute the same repaired source value into `frame_text`, `frame_text_en`, and `frame_pinyin` in the same block, so all three stay aligned with that one repaired value (though `frame_pinyin`'s substitution is the Chinese string itself, not a romanisation of it — no independent translation or romanisation occurs at these sites); sites 5-7 (the final-repair pass documented in `docs/ANSWER_SOURCE_CONTRACT.md` §3.3(4)) update `frame_text`/`counter_reply` only, with no corresponding update to English or pinyin fields at all — this remains the one specific, named source of potential Chinese/English/pinyin divergence in the whole repair chain.
...
* **Hidden Challenge-mode content remains in normal client rendering state, and learner answers are submitted normally.** Confirmed for every hidden element (§14): nothing prevents the "hidden" partner Chinese/English/options from being fully processed into DOM/client variables, and nothing prevents the learner's own answer from being submitted through whichever mechanism produced it. Whether any of this becomes *persistent* is conditional on the separate, opt-in session-capture mechanism (§16) — this is a distinction between in-memory client state and persistent storage, not a claim that Challenge Mode itself persists anything. *Observed — by design, but worth flagging given the visibility/storage distinction.*
...
| Client-intercepted spoken recovery | `matchSpokenRecoveryPhraseExact()` (`ui/app.js:2331-2350`) | Mechanism 1 only — no equivalent for Mechanism 2 | Not reached — no server request | Client `conversationTranscript`; may be included in opt-in end-session capture (`MANDARINOS_SESSION_CAPTURE=1`), but absent from `/api/run_turn` and per-turn diagnostics (§16) | `tests/verify_spoken_recovery_exact_match.js` (hybrid — mirrored matcher + static wiring) |
