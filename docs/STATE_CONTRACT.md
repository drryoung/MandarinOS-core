...
E4 handoff had no effect on the ordinary conversation path (originally documented as
SIC-6 in Section 16.2's open-gap list; now resolved and moved to Section 16.3). Commit
`3be0315b2c9f7316b03ac2183a887f602ae9a297` closes that gap by adding
...
The "exactly one transitional response whose accompanying frame still belongs to the old engine" framing in `CONVERSATION_ARCHITECTURE.md` Section 8 describes the *server-side* mechanism correctly; the client-side pickup on the following turn is now confirmed to occur for Pattern-A turns as well, closing the gap this document previously surfaced as SIC-6 (Section 16.3 records this as a resolved historical defect for traceability; it is no longer in the open-gap list, Section 16.2).
...
| `recent_confusion_count` | Client-incremented, server-read | Server resets on repair success, writes reset to `state_update` | **Client → server only, not fully round-tripped.** The client sends it every turn and the server reads it; the server's reset write to `state_update` on repair success is never consumed by the client (no merge case — Section 6), so only the client-to-server direction is operational. The client's own copy is only ever incremented locally, never reset from the server. | Contributes to overload gating, does not itself preserve/advance a frame |
...
**Summary of which recovery paths preserve the current frame vs. advance it:** client-intercepted spoken recovery (Path A) always preserves the frame; server-side typed/unintercepted recovery (Path B) always runs normal frame selection and may advance. Mirror-confusion escalation and noisy-location clarification are both **intended mechanisms with an incomplete cross-turn state path, not fully operational multi-turn ladders in production**: within a single request, a stage/level can be selected and its associated function can restate, simplify, or override the frame for that one response — but because `last_mirror_topic`/`last_mirror_engine`/`mirror_confusion_count` (mirror ladder) and `location_retry_count`/`location_clarify_hint` (noisy-location) are not sent back by the production client (rows above), neither escalation's *stage* reliably advances from one production turn to the next. Do not read either mechanism as unconditionally proceeding through its stages/levels across successive turns.
...
5. **Server restart/redeploy** — in-memory `_store` cache (learner memory) and `_personas_cache` are rebuilt from disk/content files on next access. The persisted `data/learner_memory.json` and `data/progress/*.json` files survive a restart or redeploy **only when their configured path (`MANDARINOS_DATA_DIR`) resolves to storage that itself persists across restarts** — e.g. a mounted volume on Railway. This is not automatic merely because the data is written to a file rather than kept in process memory: if `MANDARINOS_DATA_DIR` instead resolves to the container's ephemeral local disk, a restart or redeploy loses these files despite the code itself having no bug (Section 1, Section 2). This document does not verify the current Railway volume configuration.
...
| Progress snapshots | Not touched | **Explicitly preserved** (tested negatively — no `localStorage.removeItem`, no snapshot deletion call) | Not touched | Preserved | Preserved **only if `MANDARINOS_DATA_DIR` resolves to persistent storage** (e.g. a mounted volume on Railway); lost if it resolves to ephemeral container disk (Section 1, Section 2) |
...
*Partial enforcement:* the server-side `max()` combination still works because `recent_confusion_count` contributes to the same `max()` and is sent by the client every turn (client → server direction operational), but it is **not fully round-tripped**: the server's reset write to `state_update.recent_confusion_count` on repair success is never consumed by the client (Section 6), so escalation is not completely broken but the ladder is less accurate than if all three inputs were genuinely live in both directions.
...
**Document status:** Approved v1 — R2 baseline — revised against the R2 baseline to close the `current_engine`/Pattern-A client-consumption gap (formerly SIC-6, resolved and moved to Section 16.3: 14 of 20 `state_update` fields consumed, up from 13; six unconsumed fields unchanged), correct the associated invariants (SINV-4, SINV-6) and transition examples, and correct a test misattribution (`test_active_turn_record_single_source_of_truth` was cited for engine-state behaviour in three places; replaced with the new E4 client-handoff regression tests). This final consistency pass additionally qualified restart persistence (Section 13), narrowed the mirror-confusion and noisy-location escalation claims (Sections 6, 11, 16.2), corrected `recent_confusion_count` round-trip terminology (SIC-3, Section 11), and moved the resolved E4 defect out of the open-gap list into Section 16.3. This is layered on top of the prior revision's corrections to `state_update` ownership/consumption claims, the Pattern-A field count, canonical-vs-auxiliary learner-memory terminology, same-tab reset gaps, and persona-domain classification, all of which remain unchanged and preserved.
