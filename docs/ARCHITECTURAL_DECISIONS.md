...
### Decision
- Answer producers compete through a **defined, ordered priority chain**, not implicit precedence; group-local blocking (a higher-priority branch suppressing a whole lower-priority group) is deliberate.
...
### Evidence and traceability
`docs/ARCHITECTURE.md` §3, §20, §21; `docs/CHANGE_CHECKLIST.md`; the six other approved R2 documents preceding this record.
...
The following accepted practices arose directly from regression recovery, or from governance the recovery/documentation programme adopted, not from a pre-planned design: explicit state ownership (ADR-007); the approved R2 documents, and once approved this record, as mandatory maintenance controls with a defined authority hierarchy (ADR-017); bounded, additive changes and the prohibition on unrelated refactoring during recovery (ADR-019); deployment SHA verification separated from functional verification (ADR-015); execution-path-based evidence classification (ADR-016); and model allocation as a cost-conscious governance policy (ADR-020).

These are labelled `Accepted — recovery safeguard` to distinguish them from deliberate product decisions (ADR-001, ADR-002) — they exist because of what regression recovery found, not because of an independent product judgement.
